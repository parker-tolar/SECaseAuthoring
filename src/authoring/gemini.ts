...
/**
 * Generic runtime model client alias.
 * Kept for backward compatibility with existing "GeminiClient" naming.
 */
export type AuthoringModelClient = GeminiClient;

/**
 * Deterministic local model client for offline/dev flows.
 * This is useful when external Gemini access is unavailable.
 */
export class LocalAuthoringModelClient implements GeminiClient {
  async generateDraft(prompt: string): Promise<Omit<CasePackage, 'case_id'>> {
    const parsed = safeParseAuthoringPrompt(prompt);
    return buildFallbackDraft(parsed);
  }
}
...
export interface LlamaLocalClientOptions {
  /**
   * Example: http://127.0.0.1:11434
   */
  baseUrl?: string;
  /**
   * Example quantized model names in Ollama:
...
export class GeminiHttpClient implements GeminiClient {
  ...
  constructor(private readonly options: GeminiHttpClientOptions) {
    ...
    this.models = normalizeModelList(options.models ?? ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-pro']);
...
/**
 * Local Llama3 (quantized) client via Ollama-compatible API.
 * Endpoint: POST /api/generate
 */
export class LlamaLocalClient implements GeminiClient {
  private readonly baseUrl: string;
  private readonly model: string;
  private readonly fetchImpl: typeof fetch;

  constructor(options: LlamaLocalClientOptions = {}) {
    this.baseUrl = options.baseUrl ?? 'http://127.0.0.1:11434';
    this.model = options.model ?? 'llama3.1:8b-instruct-q4_K_M';
    this.fetchImpl = options.fetchImpl ?? fetch;
  }
  ...
  async generateDraft(prompt: string): Promise<Omit<CasePackage, 'case_id'>> {
    ...
    const response = await this.fetchImpl(url, {
      ...
      body: JSON.stringify({
        model: this.model,
        prompt: `${prompt}\nReturn only JSON for Omit<CasePackage,'case_id'>.`,
        stream: false,
        options: {
          temperature: 0.2,
        },
    ...
    if (!response.ok) {
      throw new AuthoringFlowError('Local llama generation failed.', 'flow_execution', {
    ...
    const parsed = JSON.parse(bodyText) as OllamaGenerateResponse;
    const json = extractJson(parsed.response ?? '');
    if (!json) {
      throw new AuthoringFlowError('Local llama response did not contain valid JSON.', 'invalid_model_response', {
        model: this.model,
        body: bodyText,
      });
    }
    ...
    try {
      ...
      throw new AuthoringFlowError('Local llama JSON parsing failed.', 'invalid_model_response', {
        model: this.model,
...
function normalizeModelList(models: string[]): string[] {
  ...
  const normalized = models.map((model) => aliasMap[model] ?? model);
...
function safeParseAuthoringPrompt(prompt: string): AuthoringInput | null {
  try {
    const value = JSON.parse(prompt);
    if (value && typeof value === 'object' && 'conflict_type' in value) {
      return value as AuthoringInput;
    }
    return null;
  } catch {
    return null;
...
function buildFallbackDraft(input?: AuthoringInput | null): Omit<CasePackage, 'case_id'> {
  const conflict = input?.conflict_type?.replace(/[^a-z0-9]+/gi, '_').toLowerCase() || 'default';
  const tone = input?.narrative_tone_bounds ?? 'balanced';
  const style = input?.resolution_style ?? 'procedural';

  const chunks = [1, 2, 3, 4].map((n) => {
    const chunk_id = `chunk_${n}`;
    const node_ids = Array.from({ length: 5 }, (_, i) => `node_auto_c${n}_${i + 1}`);
    return { chunk_id, summary: `Auto-generated chunk ${n} (${conflict})`, node_ids };
  });
  ...
  const nodes = chunks.flatMap((chunk, cidx) =>
    chunk.node_ids.map((node_id, nidx) => ({
      ...
      body: `Auto-generated [${tone}] narrative for ${node_id} with ${style} resolution pressure.`,
      choices: [
        {
    ...
    objectives: [
      {
        ...
        description: `Complete Lithcalic Override minigame under ${conflict} conditions.`,
...
export function createDefaultAuthoringClient(options: {
  provider?: 'llama_local' | 'gemini';
  llama?: LlamaLocalClientOptions;
  gemini?: GeminiHttpClientOptions;
} = {}): AuthoringModelClient {
  const provider = options.provider ?? 'llama_local';
  if (provider === 'gemini') {
    if (!options.gemini?.apiKey) {
      throw new AuthoringFlowError('Gemini provider selected but apiKey missing.', 'flow_execution');
    }
    return new GeminiHttpClient(options.gemini);
  }
  return new LlamaLocalClient(options.llama);
}
